...
export default function MyPage() {
  ...
  const [user, setUser] = useState<User>({
    ...
    bookmark_num: 0,
    reviews: {
      reviews: [{ review_id: "defaultReview_id", poster_image: "/empty.png" }],
    }, // 리뷰 초기값 추가
    bookmarks: {
      musicals: [
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL}/myPage`,
          {
            headers: {
  ...
  const handleLeave = async () => {
    try {
      ...
      const accessToken = localStorage.getItem("access_token"); // 로컬 스토리지에서 access_token 가져오기
      await axios.patch(
        `${import.meta.env.VITE_BACKEND_URL}/auth/leave`,
        {},
        {
          headers: {
            Authorization: `Bearer ${accessToken}`, // Authorization 헤더에 토큰 포함
  ...
  return (
    <>
      ...
      <Wrapper>
        ...
        <MyBookmarkCaro
          musicals={
            user.bookmarks?.musicals || [
              { musical_id: "", poster_image: "/poster_basic.png" },
              {},
            ]
          }
        />
        <Row>
          ...
          <DividerText onClick={() => setIsModalOpen(true)}>
            뮤지컬리 탈퇴하기
          </DividerText>
          {isModalOpen && (
            <LeaveModal
