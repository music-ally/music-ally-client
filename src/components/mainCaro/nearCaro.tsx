...
const NearCaro: React.FC = () => {
  ...
  useEffect(() => {
    const fetchMusicals = async () => {
      try {
        ...
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/musical/near`, {
            headers: {
  ...
  useEffect(() => {
    ...
    if (newImages.length < 4) {
      const emptySlots = 4 - newImages.length;
      const filledImages = [...newImages, ...Array(emptySlots).fill("/empty.png")];
      setDisplayImages(filledImages);
    } else {
